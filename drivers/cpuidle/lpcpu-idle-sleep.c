...
static DEFINE_SPINLOCK(s_idle_sleep_lock);
/* sync with lpm3 */
...
s32 lpcpu_idle_sleep_vote(u32 modid, u32 val)
{
	...
	spin_lock_irqsave(&s_idle_sleep_lock, flags);
	if (val != 0)
		*g_idle_sleep_vote_addr |= (u32)BIT(modid);
	else
		*g_idle_sleep_vote_addr &= ~(u32)BIT(modid);
	spin_unlock_irqrestore(&s_idle_sleep_lock, flags);
...
static int __init lpcpu_idle_sleep_init(void)
{
	...
	np = of_find_compatible_node(NULL, NULL, "hisilicon,idle-sleep");
	if (np == NULL) {
